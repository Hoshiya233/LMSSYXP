function getVideoListByAjax(data) {
    $.get("/video-list"+data,function(data, status){
        ...
        var aa="<a href='/video-list'>我全都要 </a>"
        $("div#query-list").append("请选择你的LP ", aa)
        for (var i=0;i<data.performerList.length;i++) {
            var aa="<a href='/video-list?label="+ data.performerList[i] +"'>"+ data.performerList[i] +" </a>"
            $("div#query-list").append(aa)
        }
...
function searchLabel(data) {
    data = "?label=" + data
    getVideoListByAjax(data)
}

function searchPerformer(data) {
    data = "?performer=" + data
    getVideoListByAjax(data)
}